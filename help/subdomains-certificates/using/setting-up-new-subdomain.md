...
>abstract="컨트롤 패널을 사용하면 하위 도메인의 SSL 인증서를 Adobe에서 관리하도록 할 수 있습니다. CNAME을 사용하여 하위 도메인을 설정하는 경우, 도메인 호스팅 솔루션에 인증서를 생성하기 위해 인증서 레코드가 자동으로 생성되고 제공됩니다. 이 기능은 새 하위 도메인을 설정할 때만 사용할 수 있다는 점을 참고하십시오. 기존에 위임된 하위 도메인에 대한 인증서는 위임할 수 없습니다."
...
새 하위 도메인을 설정할 때 SSL 인증서를 Adobe에서 관리할 수 있습니다. 인증서가 만료되기 전에 Adobe이 인증서를 자동으로 만들고 매년 갱신하므로 이 작업이 적극 권장됩니다. 새 하위 도메인을 설정할 때만 SSL 인증서 위임을 사용할 수 있습니다. 이미 위임된 하위 도메인에는 사용할 수 없습니다.

CNAME을 사용하여 하위 도메인 위임을 설정하는 경우 Adobe은 도메인 호스팅 솔루션에 사용할 인증서 레코드를 제공하여 인증서를 생성합니다.

>[!NOTE]
>
>Adobe 관리 SSL은 사용자가 무료로 사용할 수 있는 비용 없는 기능입니다.
